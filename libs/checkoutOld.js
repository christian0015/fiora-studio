...
export const PAYMENT_METHODS = [
  {
    id:      "cash_on_delivery",
    enabled: true,
    // Règle : COD uniquement pour commandes < 250 MAD
    maxAmount: 5250,
  },
  {
    id:      "bank_transfer",
 ...
 * selon le montant (villes plus gérées ici, car COD dispo partout mais avec condition montant)
 */
export function isCODAvailable(total) {
  const cod = PAYMENT_METHODS.find(m => m.id === "cash_on_delivery")
  if (!cod || !cod.enabled) return false
  const amountOk = total < (cod.maxAmount ?? Infinity)
  return amountOk
}

// ============================================================
// MÉTHODES DE LIVRAISON — NOUVELLES RÈGLES
// ============================================================

/**
 * Calcul des frais de livraison dynamiques selon :
 * - Ville (Casablanca ou autre)
 * - Distance approximative (pour Casa : <3km ou ≥3km)
 * 
 * @param {string} city - Ville de livraison (normalisée)
 * @param {number} distanceKm - Distance en km (optionnel, défaut 0 pour Casa centre)
 * @returns {{ fee: number, note: string, nextDayDelivery: boolean }}
 */
export function calculateShippingFee(city = "Casablanca", distanceKm = 0) {
  const normalizedCity = city.toLowerCase().trim()
  
  // Casablanca
  if (normalizedCity === "casablanca" || normalizedCity === "casa") {
    const fee = distanceKm < 3 ? 40 : 60
    return {
      fee,
      note: distanceKm < 3 
        ? "Livraison Casa (<3 km) · 40 DH"
        : "Livraison Casa (≥3 km) · 60 DH",
      nextDayDelivery: false, // Casa livraison le jour même avant 18h30
    }
  }
  ...
  return {
    fee: 100,
    note: "Livraison hors Casa · 100 DH (délai +1 jour)",
    nextDayDelivery: true, // forcé au lendemain
  }
}

/**
 * Vérifie si la commande sera livrée le lendemain
 * Règle : commande après 18h30 à Casablanca → lendemain
 *         commande hors Casa → toujours lendemain (déjà géré dans calculateShippingFee)
 * 
 * @param {Date} orderTime - Date de la commande
 * @param {string} city - Ville
 * @returns {boolean}
 */
export function isNextDayDelivery(orderTime = new Date(), city = "Casablanca") {
  const normalizedCity = city.toLowerCase().trim()
  const hour = orderTime.getHours()
  const minute = orderTime.getMinutes()
  const isAfterDeadline = (hour > 18) || (hour === 18 && minute >= 30)
  
  if (normalizedCity === "casablanca" || normalizedCity === "casa") {
    return isAfterDeadline
  }
  // Hors Casa : toujours livré le lendemain (délai technique)
  return true
}

/**
 * Génère le message de livraison pour l'affichage client
 */
export function getDeliveryMessage(city, distanceKm = 0, orderTime = new Date()) {
  const { fee, note } = calculateShippingFee(city, distanceKm)
  const nextDay = isNextDayDelivery(orderTime, city)
  const eta = nextDay ? "Livraison prévue le lendemain" : "Livraison le jour même (avant 18h30)"
  return {
    fee,
    note,
    eta,
    fullMessage: `${note} · ${eta}`,
  }
}

// Méthodes d'affichage pour le front (compatibilité avec l'existant)
export const SHIPPING_METHODS = [
  {
    id:          "casablanca_short",
    city:        "Casablanca",
    label:       { fr: "Casablanca — Moins de 3 km", en: "Casablanca — Less than 3 km", ar: "الدار البيضاء — أقل من 3 كم" },
    fee:         40,
    note:        { fr: "40 DH · Livraison le jour même avant 17h", en: "40 DH · Same day delivery before 5pm", ar: "40 درهم · توصيل نفس اليوم قبل الساعة 5" },
    enabled:     true,
  },
  {
    id:          "casablanca_long",
    city:        "Casablanca",
    label:       { fr: "Casablanca — Plus de 3 km", en: "Casablanca — More than 3 km", ar: "الدار البيضاء — أكثر من 3 كم" },
    fee:         60,
    note:        { fr: "60 DH · Livraison le jour même avant 17h", en: "60 DH · Same day delivery before 5pm", ar: "60 درهم · توصيل نفس اليوم قبل الساعة 5" },
    enabled:     true,
  },
  {
    id:          "outside_casablanca",
    city:        "Other",
    label:       { fr: "Hors Casablanca", en: "Outside Casablanca", ar: "خارج الدار البيضاء" },
    fee:         100,
    note:        { fr: "100 DH · Livraison J+1", en: "100 DH · Next day delivery", ar: "100 درهم · توصيل في اليوم التالي" },
    enabled:     true,
  },
]
...
Une copie de ces instructions vous sera envoyée par email après la commande.`,
      cash_on_delivery: `
Paiement à la livraison disponible uniquement pour les commandes < 250 DH.
Une copie de ces instructions vous sera envoyée par email après la commande.`,
    },
    paymentLabels: {
    ...
    paymentSubs: {
      cash_on_delivery: "Payez en cash à réception · < 250 DH",
...
You will get a copy of these instructions to your email after placing an order.`,
      cash_on_delivery: `
Cash on Delivery (COD) is only available for orders lower than 250 DH.
You will get a copy of these instructions to your email after placing an order.`,
    },
    paymentLabels: {
    ...
    paymentSubs: {
      cash_on_delivery: "Pay cash on receipt · < 250 DH",
...
ستصلك نسخة من هذه التعليمات عبر البريد الإلكتروني بعد تقديم الطلب.`,
      cash_on_delivery: `
الدفع عند الاستلام متاح فقط للطلبات أقل من 250 درهم.
ستصلك نسخة من هذه التعليمات عبر البريد الإلكتروني بعد تقديم الطلب.`,
    },
    paymentLabels: {
    ...
    paymentSubs: {
      cash_on_delivery: "ادفع نقداً عند الاستلام · < 250 درهم",

/**
 * ╔══════════════════════════════════════════════════════════════╗
 * ║  Flora Studio — data.js  (source de vérité centrale)        ║
...
/**
 * Retourne une catégorie par slug (avec ses sous-catégories)
 ...
 * Retourne [] si la catégorie n'existe pas ou n'a pas de sous-catégories
 * @param {string} categorySlug
 */
...
/* ─────────────────────────────────────────────────────────────
   CATALOGUE FLEURS
   Nouveaux champs :
     category    → slug de la catégorie principale (ex: 'fleur-fraiche')
     subcategory → slug de la sous-catégorie      (ex: 'bouquet') — optionnel
   ───────────────────────────────────────────────────────────── */
export const flowers = [
  ...
  /* ── 6 ──────────────────────────────────────────────────── */
  {
    ...
    reviews:          12,
    category:         'plante',
    subcategory:      null, // plante n'a pas de sous-catégories
    flowerType:       'orchidee',
...
/** Toutes les fleurs (sans pagination — usage interne) */
export function getFlowers() {
  return flowers
}

/**
 * Fleurs paginées — prêt pour la transition serverless
 * @param {Object} opts
 * @param {number} opts.page     numéro de page (1-based)
 * @param {number} opts.perPage  éléments par page
 * @returns {{ data: Flower[], total: number, page: number, perPage: number, totalPages: number }}
 */
export function getFlowersPaginated({ page = 1, perPage = 12 } = {}) {
  const total      = flowers.length
  const totalPages = Math.ceil(total / perPage)
  const safePage   = Math.max(1, Math.min(page, totalPages || 1))
  const start      = (safePage - 1) * perPage
  const data       = flowers.slice(start, start + perPage)
...
/**
 * Fleurs par catégorie principale  ex: 'fleur-fraiche'
 * Remplace getFlowersByCategory (ancien slug interne)
 */
export function getFlowersByCategory(categorySlug) {
  return flowers.filter(f => f.category === categorySlug)
}
 ...
 */
export function getFlowersByCategoryAndSub(categorySlug, subcategorySlug) {
  return flowers.filter(
    f => f.category === categorySlug && f.subcategory === subcategorySlug
  )
}

/**
 * Meilleure fleur représentant une catégorie
 * (la mieux notée et populaire) — utilisée dans ExplorerFlowers
 * @param {string} categorySlug
 */
export function getBestFlowerForCategory(categorySlug) {
  const list = getFlowersByCategory(categorySlug)
...
export function searchFlowers(query) {
  ...
  return flowers
    .map(f => {
      ...
      if ((f.flowerType ?? '').toLowerCase().includes(q)) score += 7
...
/* ── Filtre multi-critères (page boutique) ───────────────────── */

/**
 * @param {Object}  params
 * @param {string}  params.query
 * @param {string}  params.occasion
 * @param {string}  params.category      slug catégorie principale
 * @param {string}  params.subcategory   slug sous-catégorie
 ...
 * @param {number}  params.page          page (1-based)
 * @param {number}  params.perPage       éléments par page (0 = pas de pagination)
 */
export function filterFlowers({
  ...
  emotion     = '',
  page        = 1,
  perPage     = 0,  // 0 = retourne tout (usage interne / desktop sans scroll infini)
} = {}) {
  let result = [...flowers]
  ...
  if (subcategory) result = result.filter(f => f.subcategory === subcategory)
